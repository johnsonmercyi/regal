...
<<<<<<< HEAD
    ...
    public function fetchResults(Request $request, $classroom){
        ...
        $term_field = 'class_students_list.'.$school->term_column();
        $classroom = Classroom::where([['id', '=', $classroom], ['school_id', '=', $formItems['schoolId']]])->first();
        $allStudentsId = DB::table('scores')->select('scores.student_id')->
                                leftJoin('class_students_list', 'class_students_list.student_id', '=', 'scores.student_id')->
        ...
        $allSubjects = DB::table('scores')->select('subject_id')->where([['academic_session_id', $formItems['sessionId']],
                                            ['school_id', $formItems['schoolId']],
                                            ['class_id', $formItems['classId']],
                                            ['term_id', $formItems['termId']]
        ])->groupBy('subject_id')->get();

        $allScores = DB::table('scores')->select('subject_id', 'student_id', 'TOTAL')->where([['academic_session_id', $formItems['sessionId']],
                                            ['class_id', $formItems['classId']],
                                            ['term_id', $formItems['termId']],
                                            ['school_id', $formItems['schoolId']],
                                            ['status', '1']
        ...
        return response()->json(['studentRes'=>$studentDetails, 'subjectRes'=>$subjectNames, 'resultRes'=>$allScores]);
    }
    ...
    public function fetchClassList(Request $request, $classroom){
        ...
        $allStudentsId = DB::table('scores')->select('student_id')->where([['academic_session_id', $formItems['sessionId']],
                                            ['school_id', $formItems['schoolId']],
                                            ['class_id', $formItems['classId']],
                                            ['term_id', $formItems['termId']],
                                            ['status', '1']
    ...
    public function fetchStudentResult(Request $request, $student){
        ...
        $selectArray1 = [['academic_session_id', $formItems['sessionId']],
                                            ['class_id', $formItems['classId']],
                                            ['term_id', $formItems['termId']],
                                            ['school_id', $formItems['schoolId']],
                                            ['status', '1']
        ...
        $resultSelectArray = [['academic_session_id', $formItems['sessionId']],
                                ['class_id', $formItems['classId']],
                                ['term_id', $formItems['termId']],
                                ['school_id', $formItems['schoolId']],
                                ['student_id', $student->id],
        ...
        $allScores = DB::table('scores')->select('subject_id', 'student_id', 'TOTAL')->
                    where($selectArray1)->get();

        $studentScores = DB::table('scores')->select('scores.*','subjects.result_order')->
                    where($resultSelectArray)->
        ...
        foreach($allLevels as $item){
            foreach($item as $croom){
                $overAllScores[] = DB::table('scores')->select('subject_id', 'student_id', 'TOTAL')->
                    where([['academic_session_id', $formItems['sessionId']],
                                    ['class_id', $croom],
                                    ['term_id', $formItems['termId']],
                                    ['school_id', '=', $formItems['schoolId']],
        ...
        $formTeacher = DB::table('form_teachers')->select('form_teachers.staff_id', 'staff.lastName', 'staff.firstName')->
                        leftJoin('staff', 'form_teachers.staff_id', '=', 'staff.id')->
                        where([
                            ['form_teachers.academic_session_id', $formItems['sessionId']],
                            ['form_teachers.class_id', $formItems['classId']],
                            ['form_teachers.school_id', $school->id],
    ...
    public function fetchClassResult(Request $request, $classroom){
        ...
        $selectArray2 = [['academic_session_id', $formItems['sessionId']],
                        ['class_id', $formItems['classId']],
                        ['term_id', $formItems['termId']],
                        ['school_id', $formItems['schoolId']],
                    ];
        ...
        // $classScores = DB::table('scores')->where($selectArray1)->get();
        $classScores = DB::table('scores')->select('scores.*','subjects.result_order')->
                    where($selectArray1)->
                    leftJoin('subjects', 'scores.subject_id', '=', 'subjects.id')->
                    orderBy('subjects.result_order', 'ASC')->get();
        // $school = $classroom->School;
    ...
    public function fetchStudentAnnual($formItems, $student){
        ...
        $studentScores = DB::table('scores')->
                    select('scores.term_id','scores.subject_id', 'scores.student_id',
                     'scores.TOTAL', 'subjects.result_order')->
                    where($resultSelectArray)->
                    leftJoin('subjects', 'scores.subject_id', '=', 'subjects.id')->
                    orderBy('subjects.result_order', 'ASC')->get();
        ...
        $studentDetails = DB::table('students')->
                        select('id', 'firstName', 'lastName', 'otherName', 'regNo')->
                        where([['id', $student->id], ['school_id', $formItems['schoolId']]])->get();
        // $subjectNames = DB::table('subjects')->select('id', 'title')->
        //                 whereIn('id', $subjectArray)->orderBy('result_order', 'ASC')->get();
        $subjectNames = DB::table('subjects')->select('subjects.id', 'subjects.title', 'staff.signature', 'student_subjects.staff_id')
                        ->leftJoin('student_subjects', 'student_subjects.subject_id', '=', 'subjects.id')
    ...
    public function fetchClassAnnual($formItems, $classroom){
        ...
        // $classScores = DB::table('scores')->where($selectArray1)->get();
        $classScores = DB::table('scores')->select('scores.term_id','scores.subject_id', 'scores.student_id',
                    'scores.TOTAL', 'subjects.result_order')->
                    where($selectArray1)->
                    leftJoin('subjects', 'scores.subject_id', '=', 'subjects.id')->
                    orderBy('subjects.result_order', 'ASC')->get();
        ...
        $classComments = DB::table('termly_comments')->
            where($selectArray2)->get();
        // $traitRating = DB::table('school_trait_rating')->select('description', 'rating')->where('school_id', $school->id)->get();
...
>>>>>>> 69cf2848804119b50363189906bfef36757d2d0e
}
