...
/***********RUNCTION TO BUIILD FINAL RESULT SHEET******************** */
    function resultSheetFunction(resp, positionedRes, overAllResult){
        ...
        if(resp.studentComments[0]){passOrFail =  !checkAnnual ? 
        ...
        let resultHeader = `<table class='headerTable' border='1' style='font-size:13px;'>
                        <tr><th>NAME:</th><td>${studentName.toUpperCase()}</td><th>MARK OBTAINABLE:</th><td>${marksObtain}</td></tr>
                        <tr><th>CLASS:</th><td>${classChoiceDetails.class}</td><th>TOTAL SCORE:</th><td>${studentTotal.toFixed(2)}</td></tr>
                        <tr><th>REG NO:</th><td>${studentDetails[0].regNo}</td><th>AVERAGE:</th><td>${studentAverage}</td></tr>
                        <tr><th>SESSION:</th><td>${classChoiceDetails.session}</td><th>CLASS POSITION:</th><td>${stdPos}</td></tr>
                        <tr><th>TERM:</th><td>${classChoiceDetails.term}</td><th>NO. IN CLASS</th><td>${positionedRes.length}</td></tr>
                        <tr><th>NEXT TERM BEGINS:</th><td>${resumptionDate}</td>
                            <th>OVERALL POSITION:</th><td>${stdOverPos}/${overAllResult[1]}</td></tr>
                        <tr><th>PASS OR FAIL:</th><td colspan='3'>${passOrFail}</td></tr>
                        </table>`;
        ...
        const resultTableHead = `<table border='1' style='font-size:11px;text-align:center;border-radius:5px;'>
                                ...
                                <th>Subject Position</th><th>Grade</th>
        ...
        let finalResultSheet =  `<div class='resultSheet'>
                                    <div class='schoolInfo'>
                                        <div class='badge'><img width='80' src=${schoolBadge}></div>
                                        <div class='schoolName'>
                                            <span>ARCHDIOCESE OF ONITSHA</span><br>
                                            <span>${schoolName}</span><br>
...
/***************FUNCTION TO BUILD COMMENTS TABLE****************** */
    function buildResultComments(comments, teachers){
        ...
        let commentsTable = `<table border='1' class='commentTable' style='font-size:11px;'>
                            ...
                            <tr><th>Head Teacher's</th><td>${teachers.schoolHead}</td><th>Principal's Remark</th>
...
function buildResult(result, termId){
    ...
    for(const student of allStudentsId()){
        ...
        let studentBuiltRes = {
            ...
            MarksObtainable: subIdSet.size * 100,
        };
